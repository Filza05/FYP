...
public class MessageObjectUI_ : MonoBehaviour
{
    ...
    private async void OnUpdateButtonClick()
    {
        ...
        EnableEditMode(false);
        await VivoxService.Instance.EditChannelTextMessageAsync(ClientSingleton.Instance.channelToJoin, m_vivoxMessage.MessageId, updatedMessage);
    }
    ...
    private async void OnConfirmDeleteButtonClick()
    {
        ...
        await VivoxService.Instance.DeleteChannelTextMessageAsync(ClientSingleton.Instance.channelToJoin, m_vivoxMessage.MessageId);
    }
    ...
    public void SetTextMessage(VivoxMessage message, bool deleted = false)
    {
        ...
        if (deleted)
        {
            MessageText.text = string.Format($"<color=Black><size=8>{editedText}{message.ReceivedTime}</size></color>");
